...
- Protected-resource paths must not include credential query parameters such as
  ...
  in the signed request header or local settings. Names are checked at every
  percent-decoding layer.
- Malformed percent escapes, recursively encoded authority-like paths,
  backslash traversal, and excessive encoding layers are rejected before OAuth
  signing or HTTPS connection creation.
...
- Token-cache reads are limited to 16 KiB and verify that the opened descriptor
  is the same file that passed path validation. Cache directories must be real,
  current-user-owned, and not writable by group or other users.
- Token refreshes write and fsync an owner-only staging file in the cache
  ...
  the last valid token and hard-linked targets are not overwritten. Publication
  rechecks destination identity and syncs the containing directory.
