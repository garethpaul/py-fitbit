...
ATTACKER_ROOT=/tmp/py-fitbit-attacker-root
TEMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/Py Fitbit's [gate] XXXX")
...
for target in build check docs lint root-test test verify; do
